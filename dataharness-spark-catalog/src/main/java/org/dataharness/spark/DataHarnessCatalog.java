...
public class DataHarnessCatalog implements TableCatalog {
    ...
    public Table loadTable(Identifier ident) throws NoSuchTableException {
        if (Arrays.equals(DATA_HARNESS_NAMESPACE, ident.namespace())) {
            LoadTableResponse response = grpcClientFactory.getStub().loadTable(
                    LoadTableRequest.newBuilder().setTableName(ident.name()).build());
    ...
    public Identifier[] listTables(String... namespace) {
        ...
        try {
            var response = grpcClientFactory.getStub().listTables(
    ...
    public boolean tableExists(Identifier ident) {
        ...
        return grpcClientFactory.getStub().tableExists(
                TableExistsRequest.newBuilder().setTableName(ident.name()).build()).getExists();
    }
